...
contract VouchMarket {
    ...
    struct Proposal {
        ...
        uint256 amount; //Transactions cost + incentive
    ...
    constructor(uint256 _feeDivisor, uint64 _cooldownTime) {
    ...
    function claimRewardForVouching(uint64 idProposal) external {
        ...
        require(
            status == uint8(2) || registered, //status == 2 is pending registration
            "Can't claim yet"
        );
    ...
    function pay(uint64 idProposal) private {
        ...
        (bool successTx1, ) = maintainer.call{value: fee}("");
     ...
     */
    function changeParameters(uint256 _feeDivisor, uint64 _cooldownTime)
        external
        onlyMaintainer
    {
